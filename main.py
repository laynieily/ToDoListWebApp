...
def update_action():
    ...
    tk.Label(update_window, text="Select a task to update:", font=("Arial", 12, "bold")).pack(pady=5)

    cursor.execute("SELECT id, title, description, due_date, notify_days FROM tasks")
    all_tasks = cursor.fetchall()

    if not all_tasks:
        tk.Label(update_window, text="No tasks found.", font=("Arial", 10)).pack(pady=10)
        return

    task_listbox = tk.Listbox(update_window, width=50, height=10)
    task_listbox.pack(pady=5)

    for task in all_tasks:
        task_listbox.insert(tk.END, f"{task[0]} - {task[1]} (Due: {task[3]})")

    def load_selected_task():
        selection = task_listbox.curselection()
        if not selection:
            messagebox.showerror("Error", "Please select a task to update.", parent=update_window)
            return
        ...
        description_entry = tk.Text(edit_window, height=2, width=40)
        description_entry.insert(tk.END, task[2])
        description_entry.pack(pady=5)

        tk.Label(edit_window, text="Due Date (DD/MM/YYYY):").pack()
        due_date_entry = tk.Entry(edit_window, width=20)
        due_date_entry.insert(0, task[3])
        due_date_entry.pack(pady=5)

        tk.Label(edit_window, text="Notify me (days before due date):").pack()
        notify_days_entry = tk.Entry(edit_window, width=10)
        notify_days_entry.insert(0, str(task[4]))
        notify_days_entry.pack(pady=5)
        ...
        def save_updates():
            ...
            new_notify_days = notify_days_entry.get().strip()

            if not new_title or not new_due_date or not new_notify_days:
                messagebox.showerror("Error", "Title, Due Date, and Notify Days are required!", parent=edit_window)
                return

            try:
                datetime.strptime(new_due_date, "%d/%m/%Y")
                new_notify_days = int(new_notify_days)
            except ValueError:
                messagebox.showerror("Error", "Invalid Date or Notify Days!", parent=edit_window)
                return

            cursor.execute("""
                UPDATE tasks SET title = ?, description = ?, due_date = ?, notify_days = ?
                WHERE id = ?
            """, (new_title, new_description, new_due_date, new_notify_days, task[0]))
            conn.commit()
            messagebox.showinfo("Success", "Task updated successfully!", parent=edit_window)
            edit_window.destroy()
            update_window.destroy()

        submit_button = tk.Button(edit_window, text="Save Changes", command=save_updates, width=20, bg="blue", fg="white")
        submit_button.pack(pady=10)

    tk.Button(update_window, text="Edit Selected Task", command=load_selected_task, width=25, bg="blue", fg="white").pack(pady=10)
